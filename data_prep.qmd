...
そのためtxtファイルをcsvファイルに変換し，その上で国ごとにファイルを分割するという
作業が必要になります。
以下では，（1）PISA2012までのtxtファイルをcsvファイルに変換する方法，
（2）すべての国のデータを国ごとに分割する方法，の2つを解説します。

### PISA2012までのtxtファイルをcsvファイルに変換する方法

すべての国のデータが保存された巨大なtxtファイルを扱うという作業は，Rではちょっと面倒です。
そこで，Pythonを使ってtxtファイルをcsvファイルに変換します。
...
「spsファイルを使ってtxtファイルをcsvファイルに変換するプログラム」
（`txt2csv_sps.py`）をダウンロードし任意のフォルダに配置します。
さらに同じフォルダに，PISAのtxtファイルと，txtファイルをcsvに変換するためのspsファイルを配置してください。
