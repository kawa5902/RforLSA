...
（約）95\%信頼区間を考えてみると，
48.45±2×0.25≒47.95〜48.95で，母平均（=50）を捉えそこねています。
...
（約）95\%信頼区間を考えると，
48.45±2×1.01≒46.45〜50.45なので，こちらは母平均を捉えています。
...
標本を80個作るのに分母が20なのは，PISAが採用するFayのBRR（Fayの係数0.5）に由来し，
$80 \times (1 - 0.5)^2 = 20$として求まる値だからです。
...
`PV1READ`の標準偏差や標準偏差の標準誤差も，ウェイトを使って計算します。
以下では，ウェイトを考慮した分散を計算する`weighted_var`関数を自作し，
Replication Weightsと組み合わせて，`PV1READ`の標準偏差（分散の平方根）とその標準誤差を算出しています。
...
intsvyについては，[intsvyを使う](intsvy.qmd)で解説します。
